/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package abaloneserver;

import game.Message;
import java.io.IOException;

import java.net.ServerSocket;
import java.net.Socket;
import java.util.ArrayList;
import java.util.concurrent.Semaphore;
import java.util.logging.Level;
import java.util.logging.Logger;

/**
 *
 * @author Furkan ATAK
 */
//client gelişini dinleme threadi
class ServerThread extends Thread {

    public void run() {
        //listen while server is closed
        while (!Server.serverSocket.isClosed()) {
            try {
                Server.Display("Waiting For Client...");
                //wait till client come
                Socket clientSocket = Server.serverSocket.accept();
                Server.Display("Client came...");
                //make a sclient object from coming client
                //give id to it
                SClient nclient = new SClient(clientSocket, Server.IdClient);
                Server.IdClient++;
                //add client to the list
                Server.Clients.add(nclient);
                //start client message listening 
                nclient.listenThread.start();
            } catch (IOException ex) {
                Logger.getLogger(ServerThread.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
}

public class Server {

    //need to add server socket
    public static ServerSocket serverSocket;
    public static int IdClient = 0;
    //the port server is gonna listen
    public static int port = 0;
    //the thread object makes server always listen
    public static ServerThread runThread;
    //public static PairingThread pairThread;

    public static ArrayList<SClient> Clients = new ArrayList<>();

    //semafor object
    public static Semaphore pairTwo = new Semaphore(1, true);

    // give just the port number to start
    public static void Start(int openport) {
        try {
            Server.port = openport;
            Server.serverSocket = new ServerSocket(Server.port);
            Server.runThread = new ServerThread();
            Server.runThread.start();

        } catch (IOException ex) {
            Logger.getLogger(Server.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public static void Display(String msg) {

        System.out.println(msg);

    }

    //sending messages from server to clients
    public static void Send(SClient cl, Message msg) {   
        try {
            cl.sOutput.flush();
            cl.sOutput.writeObject(msg);
        } catch (IOException ex) {
            Logger.getLogger(SClient.class.getName()).log(Level.SEVERE, null, ex);
        }

    }

}
